...
int main(int argc, char *argv[]) {
	int n=9, j;//n valor modificable segun el caso*******
	int N = 4 * (n - 1) + 1; // N total
	double x[9]={1,1.3,1.4,1.45,1.6,1.72,1.8,1.93,2};
	double y[9]={5.381,8.672,9.592,9.988,10.853,11.146,11.134,10.780,10.435};
	double m[4 * (9 - 1) + 1][4 * (9 - 1) + 1],b[4 * (9 - 1) + 1]; // [N] modificar valor!! ********
	double aux;
	...
	// # Derivadas Primeras
	// j es la cantidad de condiciones para la derivada primera
	j=2*(n-1);
	for(int i=2; i<=n-1; i++)
	{
		m[j+i-1][(i-1)*4-3]=0;
	...
	/*
	Condicion de Splain natural (se puede cambiar!! OJO) (recordar que aca lo estoy igualando a la derivada segunda = 0!! 
	*/
	j=2*(n-1)+(n-2)+(n-2);
	m[j+1][1]=0;
	m[j+1][2]=0;
	m[j+1][3]=2;
	m[j+1][4]=6*x[0];
	b[j+1]=0; //Primero igual a 0
	
	m[j+2][N-4]=0;
	m[j+2][N-3]=0;
	m[j+2][N-2]=2;
	m[j+2][N-1]=6*x[n-1];
	b[j+2]=0; //Segundo igual a 0
	...
	double suma;
	double a[4 * (9 - 1)]; //vector de soluciones // a[4 * (n - 1)] // a[N-1] // a[Nnuevo] //modificar!!!!!!!!!!!!!!
